...
function renderGraphAll() {
  ...
  const min = -6;
  const max = 6;
  svg.setAttribute("viewBox", `${min} ${-max} ${max - min} ${max - min}`);
  ...
  for (let i = min; i <= max; i++) {
    ...
    v.setAttribute("x1", i);
    v.setAttribute("y1", -max);
    v.setAttribute("x2", i);
    v.setAttribute("y2", max);
    ...
    h.setAttribute("x1", min);
    h.setAttribute("y1", i);
    h.setAttribute("x2", max);
    h.setAttribute("y2", i);
  ...
  ax.setAttribute("x1", min);
  ax.setAttribute("y1", 0);
  ax.setAttribute("x2", max);
  ...
  ay.setAttribute("y1", -max);
  ay.setAttribute("x2", 0);
  ay.setAttribute("y2", max);
  ay.setAttribute("stroke", "#111827");
  ay.setAttribute("stroke-width", "0.08");
  svg.appendChild(ay);

  // Farben für die fünf Geraden
  const colors = ["#22c55e", "#2563eb", "#f97316", "#ec4899", "#a855f7"];

  funktionen.forEach((f, idx) => {
    const color = colors[idx % colors.length];

    const x1 = min;
    const y1Math = f.m * x1 + f.n;
    const x2 = max;
    const y2Math = f.m * x2 + f.n;

    const line = document.createElementNS(SVG_NS, "line");
    line.setAttribute("x1", x1);
    line.setAttribute("y1", y1Math);
    line.setAttribute("x2", x2);
    line.setAttribute("y2", y2Math);
    line.setAttribute("stroke", color);
    line.setAttribute("stroke-width", "0.09");
    svg.appendChild(line);

    // y-Achsenabschnitt als Punkt markieren
    const y0 = f.n;
    if (y0 >= min && y0 <= max) {
  ...
  if (legendDiv) {
    ...
    funktionen.forEach((f, idx) => {
      ...
      item.innerHTML = `
        ...
        <span>(${f.id}) <code>${funktionsText(f)}</code></span>
      `;
...
function buildAufgabe1() {
  ...
  container.innerHTML = `
    ...
    <div class="card" id="task-b">
      ...
      <div class="subtask-body">
        <p>
          Im Koordinatensystem oben sind bereits alle Funktionen (1)–(5) eingezeichnet.
          Beziehe dich dort besonders auf die Geraden von (1) und (2).
        </p>
  ...
  attachSolutionHandlers(f1, f2, f4, f5, t1, t2);
}
...
function prepareTaskGandH(f4, f5) {
  ...
  const yS = onGraph ? yCorrect : yCorrect + offset;
...
function attachSolutionHandlers(f1, f2, f4, f5, t1, t2) {
  const buttons = document.querySelectorAll("[data-solution-btn]");
  buttons.forEach((btn) => {
    const id = btn.getAttribute("data-solution-btn");
    btn.addEventListener("click", () => handleSolutionClick(id, f1, f2, f4, f5, t1, t2));
  });
}
...
function handleSolutionClick(id, f1, f2, f4, f5, t1, t2) {
  switch (id) {
    case "a1":
      checkA("a1", f1, t1);
      break;
...
function checkA(taskId, f, table) {
  const inputsY = document.querySelectorAll(`input[data-task="${taskId}"][data-role="y"]`);
  const mInput = document.querySelector(`input[data-task="${taskId}"][data-role="m"]`);
  const nInput = document.querySelector(`input[data-task="${taskId}"][data-role="n"]`);
  ...
  const msgParts = [];
  msgParts.push(allYCorrect ? "Wertetabelle stimmt." : "In der Wertetabelle sind noch Fehler.");
  ...
  const rows = table
    .map(
      (p) =>
        `x = ${p.x}  →  y = ${f.m}·(${p.x}) ${f.n >= 0 ? "+ " + f.n : "- " + Math.abs(
          f.n
        )} = ${p.y}`
    )
    .join("<br>");
...
// b) Graphen qualitativ erklären
function explainB(f1, f2) {
  const exp = `
    <strong>Graph von Funktion (1):</strong><br>
    <code>${funktionsText(f1)}</code><br>
    Die Gerade hat die Steigung m = ${f1.m} – bei jedem Schritt 1 nach rechts geht sie
    ${f1.m > 0 ? "nach oben" : "nach unten"}.<br>
    Sie schneidet die y-Achse im Punkt (0 | ${f1.n}).<br><br>

    <strong>Graph von Funktion (2):</strong><br>
    <code>${funktionsText(f2)}</code><br>
    Steigung m = ${f2.m}, y-Achsenabschnitt n = ${f2.n}.<br>
    Im Koordinatensystem oben kannst du sehen, wie die Geraden aussehen:
    Trage die Punkte aus der Wertetabelle ein und verbinde sie.
  `;
  setFeedback("b", "", true);
...
function checkC(f2) {
  ...
  let msg = ok ? "✅ Beide Schnittpunkte sind korrekt." : "❌ Mindestens ein Schnittpunkt ist noch falsch.";
...
// f) Gemeinsame Eigenschaft
function explainF(f1, f4) {
  let eigenschaft;
  if (f1.m > 0 && f4.m > 0) eigenschaft = "Beide sind streng monoton steigend (m > 0).";
  else if (f1.m < 0 && f4.m < 0)
    eigenschaft = "Beide sind streng monoton fallend (m < 0).";
  else eigenschaft = "Beide sind Geraden (lineare Funktionen) und haben genau eine Nullstelle.";
  ...
  `;
  setFeedback("f", "ℹ️ Deine Formulierung wird nicht automatisch bewertet.", true);
...
function checkG(f4) {
  ...
  const ok = (saysYes && reallyOnGraph) || (!saysYes && !reallyOnGraph);
  const fb = ok
  ...
  const exp = `
    ...
    y = ${f4.m}·${taskGPoint.x} ${f4.n >= 0 ? "+ " + f4.n : "- " + Math.abs(
    f4.n
  )} = ${fVal}.<br>
    Da der y-Wert von S ${taskGPoint.y} ist, gilt:<br>
    ${Math.abs(fVal - taskGPoint.y) < 1e-6 ? "f(x) und y sind gleich ⇒ S liegt auf dem Graphen." : "f(x) und y sind verschieden ⇒ S liegt nicht auf dem Graphen."}
  `;
...
function checkH(f5) {
  ...
  const exp = `
    ...
    y = ${f5.m}·${taskHP.x} ${f5.n >= 0 ? "+ " + f5.n : "- " + Math.abs(
    f5.n
  )} = ${taskHP.y}.<br>
    Also P(${taskHP.x} | ${taskHP.y}).<br><br>
    Punkt Q hat y = ${taskHQ.y}. Wir lösen nach x:<br>
    ${taskHQ.y} = ${f5.m}x ${f5.n >= 0 ? "+ " + f5.n : "- " + Math.abs(
    f5.n
  )}<br>
    ⇒ ${f5.m}x = ${taskHQ.y} ${f5.n >= 0 ? "- " + f5.n : "+ " + Math.abs(
    f5.n
  )} = ${f5.m * taskHQ.x}<br>
...
function explainI(f1) {
  ...
  `;
  setFeedback("i", "ℹ️ Deine Beschreibung wird nicht automatisch bewertet.", true);
