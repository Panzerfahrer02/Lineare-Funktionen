...
function buildAufgabe1() {
  ...
  container.innerHTML = `
    ...
    <div class="card" id="task-b">
      ...
      <div class="subtask-body">
        <p>
          Überlege, wie die Graphen der Funktionen (1) und (2) im Koordinatensystem aussehen würden.
          Nutze dafür die Steigungen und y-Achsenabschnitte aus Teilaufgabe a).
        </p>
        <div class="coord-placeholder">
          Stell dir hier ein Koordinatensystem vor: Trage die Punkte aus der Wertetabelle ein und
          verbinde sie zu Geraden. Funktion (1) und (2) liegen im gleichen Koordinatensystem.
        </div>
  ...
  // Zusatzdaten für g) und h): wir wählen zufällige Punkte passend zu f4/f5
...
function prepareTaskGandH(f4, f5) {
  // g) Punkt S – wir wählen einen x-Wert und entscheiden zufällig, ob der Punkt draufliegt
  const xS = randomInt(-3, 3);
  const onGraph = Math.random() < 0.5;
  const yCorrect = f4.m * xS + f4.n;
  const yS = onGraph ? yCorrect : yCorrect + randomInt(-3, 3) || (yCorrect + 1);
...
function explainB(f1, f2) {
  const exp = `
    ...
    Auch hier kannst du die Punkte aus der Wertetabelle in ein Koordinatensystem eintragen und
    dann die Punkte mit einer Geraden verbinden.
  `;
  setFeedback("b", "", true);
...
// i) Monotonie & Symmetrie von (1) – reine Erklärung
